...
export default {
  async fetch(request, env) {
    ...
    // The Worker serves the website for normal browser requests and handles
    // contact-form submissions through the API endpoint below.
    if (url.pathname === '/api/contact') {
      return handleContact(request, env);
    }

    if (request.method !== 'GET' && request.method !== 'HEAD') {
      return json({ ok: false, error: 'Method not allowed.' }, 405, getCorsOrigin(request));
    }
...
async function handleContact(request, env) {
  ...
  if (!contentType.includes('application/json')) {
    return json({ ok: false, error: 'Expected JSON.' }, 415, origin);
  }
  ...
  if (!name || !email || !message) {
    return json({ ok: false, error: 'Please complete the required fields.' }, 400, origin);
  }
  ...
  if (!env.RESEND_API_KEY) {
    return json({ ok: false, error: 'Email service is not configured.' }, 500, origin);
  }
